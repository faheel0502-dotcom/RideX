    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-canvas px-4 py-12 sm:px-6 lg:px-8 transition-colors duration-300">
      ...
      <div className="w-full max-w-md space-y-8 bg-surface-raised border border-border/40 p-8 rounded-cards shadow-2xl">
        ...
        <div className="text-center">
          ...
          <h2 className="mt-4 font-display text-xl font-bold text-text-primary uppercase tracking-wide">Create Rider Account</h2>
          <p className="mt-2 text-xs text-text-secondary">
            Join the pack. Save shipping details and track order dispatching.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          ...
          >
            ...
            <span>{formLoading ? 'Creating Account...' : 'Sign Up'}</span>
          </button>
        </form>

        <div className="text-center text-xs text-text-secondary pt-4 border-t border-border/20">
          Already have a rider account?{' '}
          <Link href={`/login?redirect=${encodeURIComponent(redirect)}`} className="text-orange-accent hover:underline font-semibold">
            Log In
          </Link>
        </div>
      </div>
